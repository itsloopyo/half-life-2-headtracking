...
// oracle's files are the published build's, taken with `git show v0.2.0:src/<file>`. The core
// files both readers compile are hash-equal to `git -C cameraunlock-core show ee8cc72:<path>`
// (and to f441e29, v0.1.0's pin), so the readers differ only where the mod's own reader changed.
...
constexpr Pinned kPinned[] = {
    ...
    {"tests/config_differential/oracle/src/config.cpp", "4e1d8021d3eb6dd584b9a887198ad8df92893bb81ef4c5487c2cd1c330d58ee8"},
    ...
    // The oracle only: the headers v0.2.0's config.h and config.cpp include beside the reader.
    ...
    // The import: src/debug_log.h is v0.2.0's, the legacy folder is frozen.
    {"src/debug_log.h", "93da8c6ce36715bc6f9327274927513998aef9c87c176e71617b89cc12fce5cb"},
    {"src/legacy_config/legacy_config.h", "9ac92e5ebb0a6a420aa287c16fec849a19c1663cbb7c9e0c1b82e8e32411acdd"},
    {"src/legacy_config/legacy_config.cpp", "9252973a79c4d2c28affda445eac50176c475c897b1d5b77422eec29380674e5"},
};
...
fs::path SourcePath(const std::string& relative) { return fs::path(HL2_SOURCE_DIR) / relative; }
...
public:
    Scratch() {
        ...
        root_ = fs::path(temp) / ("hl2_ht_diff_" + std::to_string(GetCurrentProcessId()) + "_" +
...
// reader. v0.2.0:src/hotkey_handler.cpp:25-31 (HotkeyHandler::Start): the three configured
...
// Hand copied from v0.2.0, unchanged at the frozen reader's commit: src/plugin.cpp:28-29
...
Observed ReadOracle(const std::string& dir) { return ObservePublished(hl2_published::Start(dir)); }
...
void FirstRunFileIsThePublishedBuilds() {
    ...
    if (!same) {
        std::ofstream(SourcePath("build/v0.2.0-first-run.actual.ini"), std::ios::binary) << written;
    }
    Check(same, "v0.2.0-first-run.ini is what the published build writes at first run");
}
