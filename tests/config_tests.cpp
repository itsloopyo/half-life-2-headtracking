...
// fresh render, the defaults v0.2.0 ran on map to the defaults, a first start creates the
...
// and a folder the ANSI code page cannot name imports as v0.2.0 read it.
//
...
std::string Committed() {
    return ReadBytes(fs::path(HL2_COMMITTED_CONFIG));
}
...
struct Scratch {
    ...
    explicit Scratch(const std::wstring& gameFolder) {
        ...
        root = fs::path(temp) / (L"hl2-config-tests-" + std::to_wstring(GetCurrentProcessId()));
...
// A fresh install and an upgrade from v0.2.0's defaults start the same: the map of the frozen
...
void TestLegacyDefaultsMapToTheDefaults() {
    wchar_t temp[MAX_PATH];
    GetTempPathW(MAX_PATH, temp);
    const fs::path missing = fs::path(temp) / L"hl2-no-such-folder" / kLegacyConfigFileName;
    ...
    Check(result.dropped.empty(), "v0.2.0's defaults drop nothing");
    ...
    Check(AllValues(mapped) == AllValues(table.defaults()), "v0.2.0's defaults map to the defaults");
...
// v0.2.0 took its folder from core's HostExeDirectoryNarrow, which refuses a folder the ANSI
...
// it was.
void TestAFolderTheCodepageCannotNameImportsAsV020ReadIt() {
    ...
    Check(loaded.status == cfg::ConfigLoadStatus::Migrated, "a file v0.2.0 never read migrates to its defaults");
    Check(loaded.config.udp_port == 4242, "the port is v0.2.0's default, as that build ran");
    Check(ReadBytes(s.LegacyPath()) == legacyBytes, "HeadTracking.ini stays as it was");
}

// [View] Fov and FovViewmodel take 0 or 30 to 150, as v0.2.0 did. A value outside keeps the
...
int main(int argc, char** argv) {
    try {
        ...
        TestTogglesSave();
        TestAFolderTheCodepageCannotNameImportsAsV020ReadIt();
