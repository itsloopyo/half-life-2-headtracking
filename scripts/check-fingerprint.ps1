...
<#
Prints the PE fingerprint of the installed Half-Life 2 client.dll as a
...
because that is the module CViewRender::RenderView lives in.
...
param(
    [string]$GamePath,
    [string]$DllRelPath = 'hl2\bin\client.dll'
)
...
if (-not $GamePath) {
    $GamePath = Find-GamePath -GameId 'half-life-2'
    if (-not $GamePath) { throw "Half-Life 2 not found. Pass -GamePath explicitly." }
}
...
Write-Host "    { 0x0u, kViewSetupLayout_2025 },  // RenderView RVA - rederive before use"
Write-Host "};"
Write-Host ""
...
Write-Host "   fits before reusing kViewSetupLayout_2025; if it moved, add a new layout."
Write-Host ""
