...
DWORD WINAPI BootstrapThread(LPVOID) {
    try {
        ...
        if (exeDir.empty()) {
            ...
            HT_LOG("[main] HalfLife2HeadTracking %s: Windows reported no folder for hl2.exe, so "
        ...
        HT_LOG("[main] HalfLife2HeadTracking %s loaded into pid %lu",
...
BOOL APIENTRY DllMain(HMODULE hModule, DWORD reason, LPVOID /*reserved*/) {
    switch (reason) {
        case DLL_PROCESS_ATTACH:
            ...
            // "HalfLife2HeadTracking.asi_unloaded", usually
